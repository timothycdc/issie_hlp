...
let satTimes (a: int32) (b: int32) =
    if int64 a * int64 b > int64 System.Int32.MaxValue then
...
// Takes a data structure that represents a cyclic list.
// It returns a value of a specific data structure that has two properties: Data and Size.

let fromArray (l: 'a array) =
    { Data = (fun i -> l[i % l.Length]); Size = l.Length }

let toArray (g: Gen<'a>) = Array.init g.Size g.Data

let toList (g: Gen<'a>) = List.init g.Size g.Data

/// return a shuffled range of integers
let randomInt (min: int) (step: int) (max: int) : Gen<int> =
    [| min..step..max |] |> shuffleA |> fromArray
...
let product (f: 'a1 -> 'a2 -> 'b) (g1: Gen<'a1>) (g2: Gen<'a2>) : Gen<'b> =
    ...
    if g1.Size > Constants.maxFirstProductArgSize then
        printfn
            $"Warning: product of sequence sizes: {g1.Size} X {g2.Size}. {g1.Size} \
                will take a like time to cycle through {g1.Size} values of g1 before advancing g2"
    { Data = (fun i -> f (g1.Data <| i % n1) (g2.Data <| (i / n1) % n2))
      Size = satTimes n1 n2 }
...
let truncate (size: int) (samples: Gen<'a>) = { samples with Size = size }
